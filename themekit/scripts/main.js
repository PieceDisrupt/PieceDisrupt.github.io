...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    sections_slider_parallax = $(body).find(
      " > main > .section-slider[data-slider-parallax]"
    );
    ...
    if (window_width > 992) {
      ...
      $(body).on("click", ".menu-cnt > ul > li > a,.lan-menu a", function () {
        ...
        $(body)
          .find(" > nav .menu-cnt > ul > li,.shop-menu-cnt,.lan-menu li")
          .removeClass("active");
      ...
      if ($(nav).hasClass("menu-fixed")) {
        $(nav)
          ...
          .css(
            ...
            window_height -
              5 -
              $(nav).find(" > .container").outerHeight() +
              "px"
          );
      }
    }
    ...
    onePageScroll($(".menu-inner,.menu-one-page, .main-content"));
    ...
    $(".section-video,.section-block .block-media").each(function () {
      $(this)
        .find(" > video")
        .set_video_size($(this).width(), $(this).height());
    });
    ...
    if (scroll_detect.length) {
      $(scroll_detect).each(function () {
        ...
        if (target.length > 1 && target.indexOf("#") == 0) {
          scroll_detect_items.push(
            $(body).find($(this).find("a").attr("href"))
          );
        }
      });
    }
